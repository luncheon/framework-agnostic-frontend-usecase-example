...
const WorksheetCell: React.SFC<{ rowIndex: number, columnIndex: number, active: boolean, content?: string }> =
...
const WorksheetRow: React.SFC<{ rowIndex: number, row?: string[], activeCellIndex?: number, maxColumnIndex: number }> =
...
const WorksheetTableHeader: React.SFC<{ maxColumnIndex: number }> =
  ({ maxColumnIndex }) => (
    <thead>
      <tr>
        <th></th>
...
const WorksheetTableHeaderMemoized = React.memo(WorksheetTableHeader)

const WorksheetTable: React.SFC<{ worksheet: Worksheet, worksheetOperations: WorksheetOperations }> =
  ({ worksheet: { maxCellAddress, activeCellAddress, cellContents }, worksheetOperations }) => (
    <table className='worksheet'>
      <WorksheetTableHeaderMemoized maxColumnIndex={maxCellAddress.columnIndex} />
      <tbody
...
export default () => {
  ...
  return <WorksheetTable worksheet={state} worksheetOperations={worksheetOperations} />
}
